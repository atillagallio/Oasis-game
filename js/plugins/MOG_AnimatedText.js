 ...
 * EG
 *  
 * infotext : 1 : 0 : 200 : 200 : 20 : 120 : 10 : 0 : 0 : Level UP!
 *
 * ----------------------------------------------------------------------------
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    ...
    if (command === "infotext")  {
		...
		mode = Number(args[3]);
		var x = ""
		for (i = 5; i < 6; i++){x += args[i] + " "};


		if (x.toString().includes("EV#"))  {	          
			  	$gameMap.events().forEach(function(event) {
					if (!event._erased && event.eventId() === Number(x.toString().substr(3))) {
						x = event.screenX() - 50;
		...
		var y = ""
		for (i = 7; i < 8; i++){y += args[i] + " "};

		if (y.toString().includes("EV#"))  {	          
		  	$gameMap.events().forEach(function(event) {
				if (!event._erased && event.eventId() === Number(y.toString().substr(3))) {
					y = event.screenY() - 50;
				};
	    	}, this);
	    }  			
...
InfoText.prototype.setBaseParameters = function(i,dir,letter,itemMax) {
		 ...
		 //Removing outline
		 this._letters[i]._bitmap.outlineWidth = 0;
};
